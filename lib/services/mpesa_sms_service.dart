...
class MpesaSmsService {
  ...
  models.Transaction? parseMpesaSms(SmsMessage message) {
    try {
  ...
  Future<List<models.Transaction>> importAllMpesaTransactions() async {
    final messages = await getMpesaSms();
    final importedTransactions = <models.Transaction>[];
    
    for (final message in messages) {
      final transaction = parseMpesaSms(message);
      if (transaction != null) {
  ...
  Future<List<models.Transaction>> previewMpesaTransactions() async {
    final messages = await getMpesaSms();
